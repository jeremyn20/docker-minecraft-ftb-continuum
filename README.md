...
To simply use the latest stable version, run:
    ...
    docker run -d -p 25565:25565 mattiaskagstrom/minecraft-ftb-skyfactory3
...
Speaking of multiple servers, it's handy to give your containers explicit names using `--name`, such as
    ...
    docker run -d -p 25565:25565 --name minecraft mattiaskagstrom/minecraft-ftb-skyfactory3
...
In order to persist the Minecraft data, which you *probably want to do for a real server setup*, use the `-v` argument to map a directory of the host to ``/data``:
    ...
    docker run -d -v /path/on/host:/data -p 25565:25565 mattiaskagstrom/minecraft-ftb-skyfactory3
